...
% ================= 坐标轴设置 =================
% 原始范围
x_min = datenum_dates(1);
x_max = datenum_dates(end);
y_min = -0.9;
y_max = 0.9;
...
xlim([x_min - 0.015*x_range, x_max + 0.015*x_range]);
ylim([y_min - 0.02*y_range, y_max + 0.02*y_range]);
...
set(gca, 'LooseInset', [0 0 0 0]);   % 去掉自动留白
set(gca, 'Position', [0.005 0.005 1 1]);     % 让图铺满整个画布
set(gcf, 'Renderer', 'painters');   % 关键：强制矢量渲染
print(gcf, 'figure.svg', '-dsvg');